...
func main() {
	...
	addresses, err := GetAddress("final-address.txt")
	if err != nil {
		panic(err)
	}

	fmt.Println("address to find:", len(addresses))

	svc := NewService(words, addresses, db)
	svc.FindMnemonic(12)
}
...
// ================= INTERNAL =================
func GetAddress(path string) (map[string]struct{}, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
